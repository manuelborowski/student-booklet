...
from . import db
from .models import Remark, Student, ExtraMeasure
...
def db_filter_remarks_to_be_reviewed(schoolyear, test_remark = False, commit = True):
    students = db.session.query(Student).join(Remark).filter(Remark.reviewed == False, Student.schoolyear == schoolyear, Remark.test == test_remark). \
