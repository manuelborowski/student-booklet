...
def load_user(user_id):
    ...
    teacher = db_teacher.db_teacher(code=user.username)
    user.in_schedule = not not db_schedule.db_schedule_list(teacher=teacher) if teacher else False
    user.in_replacement = not not db_replacement.replacement_list(id=teacher.id) if teacher else False

    teacher_ids = []
    if user.in_schedule:
        teacher_ids.append(teacher.id)
    if user.in_replacement:
        teacher_ids += db_replacement.replacement_list(teacher.id, ids_only=True)
    if teacher_ids:
