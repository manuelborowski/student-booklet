...
def process_data(table, paginate=True):
    ...
    if _model is Remark:
        ...
        _filtered_list = db.session.query(ExtraMeasure, Remark, Student, Grade).join(Remark,
                                                                                     Remark.extra_measure_id == ExtraMeasure.id) \
        ...
        _filtered_list = db.session.query(ReplacementTeacher, Teacher).join(Teacher,
                                                                            ReplacementTeacher.replaced_by_id == Teacher.id)
    else:
    ...
    if search_value:
        ...
        if Remark.concat_subjects in column_list:
            search_constraints.append(Remark.concat_subjects.like(search_value))
